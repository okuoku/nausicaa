...
;;;Date: Mon Dec  1, 2008
;;;Time-stamp: <2008-12-01 06:51:19 marco>
;;;
;;;Abstract
;;;
...
(library (uriel ffi errno)
  (export
    errno->integer	errno->integer/or-error
    integer->errno	integer->errno/or-error
  ...
  (define (errno->integer symbol)
    (assq symbol errno-alist))

  (define (errno->integer/or-error symbol)
    (let ((value (assq symbol errno-alist)))
      (or value (assertion-violation 'errno->integer/or-error
		  "expected errno symbol" symbol))))
  ...
  (define (integer->errno/or-error num)
    (let ((pair (find (lambda (pair)
			(= num (cdr pair)))
		      errno-alist)))
      (if pair
	  (car pair)
	(assertion-violation 'integer->errno/or-error
	  "expected value of errno constant" num)))))
