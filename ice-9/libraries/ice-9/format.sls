...
;;page
;; ------------------------------------------------------------
;; Utilities for porting under Ikarus Scheme.
;; ------------------------------------------------------------
...
;;Build a new list hoding the first K elements from LST.
(define (list-head lst k)
  (let loop ((result	'())
	     (source	lst))
    (if (and (= 0 k) (not (null? source)))
	(loop (- k 1) (cons (car source) result) (cdr source))
      (reverse result))))

(define (fold func default the-list)
  (let loop ([prev	default]
	     [ell	the-list])
    (if (null? ell)
	prev
      (loop (func prev (car ell)) (cdr ell)))))
...
(define (object->string obj)
  (call-with-string-output-port (lambda () (write obj))))

;;Return the position  of the first occurrence of  CHAR in STRING
;;or #f if not found.
(define (string-index string char)
  (let ([len	(string-length string)])
    (let loop ([i 0])
      (if (<= len i)
	  #f
	(if (equal? char (string-ref string i))
	    i
	  (loop (+ 1 i)))))))
