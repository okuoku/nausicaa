...
;;;
;;;Ported  from MIT Scheme  runtime by  Brian D.   Carlstrom.  Massively
;;;rehacked &  extended by Olin Shivers 6/98.   Massively redesigned and
;;;rehacked 5/2000 during SRFI process.
;;;
...
;;;This is  simple code,  not great code.  Char sets are  represented as
;;;256-char strings. If char I is  ASCII/Latin-1 0, then it isn't in the
;;;set; if char I is ASCII/Latin-1 1, then it is in the set.
;;;
...
;;;dependencies.
;;;
;;;First  and foremost, rewrite  this code  to use  bit vectors  of some
;;;sort.  This will give big speedup and memory savings.
;;;
;;;LET-OPTIONALS* macro
;;;--------------------
;;;
...
;;;
;;;Implementations  that  can arrange  for  the  base  char sets  to  be
;;;immutable should do so. (E.g., Scheme  48 allows one to mark a string
;;;as immutable, which  can be used to protect  the underlying strings.)
;;;It would be  very, very bad if a client's  buggy code corrupted these
;;;constants.
;;;
...
;;;up if an illegal value is  passed in. However, the error message will
;;;not be  as good as  if the error  were caught at the  "higher level."
;;;Also,  a very,  very smart  Scheme compiler  may be  able  to exploit
;;;having the  type checks done  early, so that  the actual body  of the
;;;procedures can assume proper values.  This isn't likely; this kind of
;;;compiler technology isn't common any longer.
;;;
...
;;;these procedures, *especially* the many optional BASE-CS parameters.
;;;
...
;;;
;;;There  is  enough  *explicit*  error  checking  that  *all*  internal
;;;operations   should   *never*   produce   a   type   or   index-range
;;;error. Period. Feel like living dangerously? *Big* performance win to
;;;be  had by replacing  string and  record-field accessors  and setters
;;;with  unsafe  equivalents  in  the code.  Similarly,  fixnum-specific
;;;operators can speed up the arithmetic done on the index values in the
;;;inner loops. The only arguments that are not completely error checked
;;;are
;;;
...
;;;default is the  empty set.  The PROC argument is  to help us generate
  ...
  (if (pair? maybe-base)
	...
	(if (null? tail)
		...
		(%string-copy (char-set:s bcs))
	      (error "BASE-CS parameter not a char-set" bcs))
	  (error "expected final base char set -- too many parameters" maybe-base)))
    (make-string 256 (%latin1->char 0))))

;;;If CS is really a char-set, do CHAR-SET:S, otw report an error msg on
;;;behalf  of  our caller,  PROC.  This  procedure  exists basically  to
;;;provide explicit error-checking & reporting.
(define (%char-set:s/check cs proc)
  (let lp ((cs cs))
    (if (char-set? cs) (char-set:s cs)
      (lp (error "Not a char-set" cs)))))
