...
(library (nausicaa posix file)
  ...
  (import (except (rnrs) remove truncate)
    ...
    (prefix (nausicaa posix file primitives) primitive:)
    (only (nausicaa posix fd) open close lseek)
    (prefix (only (nausicaa posix file platform) stat fstat sizeof-stat) platform:))
