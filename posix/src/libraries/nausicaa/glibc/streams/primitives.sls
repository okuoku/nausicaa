...

(library (nausicaa glibc streams primitives)
  (export
  ...
  (import (rnrs)
    ...
    (only (nausicaa ffi peekers-and-pokers) pointer-ref-c-pointer)
    (only (nausicaa ffi memory) malloc-small/c primitive-free)
    (only (nausicaa ffi cstrings) string->cstring/c cstring->string)
    (only (nausicaa ffi errno) raise-errno-error)
    (only (nausicaa posix sizeof) EOF)
