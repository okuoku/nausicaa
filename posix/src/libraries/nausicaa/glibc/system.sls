...
(library (nausicaa glibc system)
  (export
    ...
    (rename (primitive:unsetenv		unsetenv)
	    (primitive:clearenv		clearenv)
	    (primitive:putenv		putenv)
	    (primitive:putenv*		putenv*)))
  (import (rnrs)
    (nausicaa posix helpers)
    (prefix (nausicaa glibc system primitives) primitive:))
