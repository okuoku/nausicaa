...
;;;Contents: portable higienic pattern matcher macro transformer
...

#!r6rs
(library (syntax-matches)
  (export
    syntax-match
    &syntax-match-mismatch
    make-syntax-match-mismatch-condition
    syntax-match-mismatch-condition?
    condition-syntax-match-mismatch-expression
    syntax-match-mismatch-error)
  (import (rnrs))


(define-condition-type &syntax-match-mismatch
  &mismatch
  make-syntax-match-mismatch-condition
  syntax-match-mismatch-condition?
  (expr condition-syntax-match-mismatch-expression))

(define-syntax syntax-match-mismatch-error
  (syntax-rules ()
    ((_ ?who ?expr)
     (raise (condition (make-syntax-match-mismatch-condition ?expr)
...
(define-syntax syntax-match
  ...
  (lambda (stx)
    (syntax-case stx (=>)
      ((_)
       (syntax-violation 'syntax-match "missing match expression" (syntax-datum stx)))

      ((_ ?input-form-stx)
       (syntax-violation 'syntax-match "missing match clause" (syntax-datum stx)))

      ((_ ?input-form-stx ?clause ...)
       (identifier? #'?input-form)
       #'(match-clause ?input-form ?input-form-stx (set! ?input-form-stx) ?clause ...))
      ...
      ((_ ?atom ?clause ...)
       #'(match-clause ?atom ?atom
		       (assertion-violation 'syntax-match "invoked setter for an atom")
		       ?clause ...))
      )))
  ...
  (lambda (stx)
    (syntax-case stx (=>)
      ...
      ((_ ?expr ?Getter ?Setter (?pattern) . ?other-clauses)
       (syntax-violation 'syntax-match "no body in match clause" (syntax->datum #'(?pattern))))

      ((_ ?expr ?Getter ?Setter (?pattern (=> ?failure)) . ?other-clauses)
       (syntax-violation 'syntax-match "no body in match clause" (syntax->datum #'(?pattern (=> ?failure)))))

      ;;No more clauses, raise an error.
      ((_ ?expr ?Getter ?Setter)
       (match-mismatch-error 'syntax-match ?expr))
...
;;; --------------------------------------------------------------------
      ...
      ((_ ?expr (:not) ?Getter ?Setter ?success-kont ?failure-kont ?bound-pattern-variables)
       (syntax-violation 'syntax-match "empty :NOT form in pattern" '(:not)))
...
;;; --------------------------------------------------------------------
      ...
      ((_ ?expr ?pattern ?Getter ?Setter
	  ?success-kont ?failure-kont ?bound-pattern-variables)
       (syntax-violation 'syntax-match "unrecognised pattern element" (syntax->datum #'?pattern)))
...
(define-syntax extract-new-pattern-variables
  ...
  ;;            <body>
  ;;          (match-mismatch-error 'syntax-match (cdr expr)))))
  ;;
