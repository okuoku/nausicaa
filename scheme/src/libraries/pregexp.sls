...
;;;Contents: pregexp library by Dorai Sitaram
;;;Date: Sun Jul 12, 2009
;;;
;;;Abstract
;;;
;;;	This library is  based upon "pregexp" by Dorai  Sitaram.  It was
;;;	modified  to work with  R6RS Scheme  implementations and  to fit
;;;	into the Nausicaa distribution.
;;;
;;;	  The original code  is available at (URL last  verified Sun Jul
;;;	12, 2009):
;;;
...
;;Replace with REVERSE! from (lists).
;;
...
(define (pregexp-char-word? c)
		;too restrictive  for Scheme but  this is what \w  is in
		;most regexp notations
...
;;re  is   a  compiled  regexp.   It's   a  list  that   can't  be  nil.
;;pregexp-match-positions-aux  returns a  2-elt  list whose  car is  the
;;string-index following the matched portion and whose cadr contains the
;;submatches.  The proc returns false if there's no match.

;;Am spelling loop- as loup-  because these shouldn't be translated into
;;CL loops by scm2cl (although they are tail-recursive in Scheme)
...
(define (pregexp-replace* pat str ins)
  ...
  (let ((pat (if (string? pat) (pregexp pat) pat))
	...
	(let ((pp (pregexp-match-positions pat str i n)))
	  (if (not pp)
	      (if (= i 0)
		;this  implies pat  didn't match  str at  all,  so let's
		;return original str
		  str
		;else: all  matches already found and replaced  in r, so
		;let's just append the rest of str
		(string-append
		 r (substring str i n)))
	    (loop (cdar pp)
		  (string-append
