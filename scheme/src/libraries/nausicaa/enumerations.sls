...
;;;Contents: enumeration utilities
  ...
  (lambda (stx)
    ...
    (syntax-case stx ()
      ((_ ?name (?flag0 ?flag ...) (?symbol0 ?symbol ...))
       (with-syntax ((ENUM-NAME		(datum->syntax #'?name
						       (%name->enum-name      (syntax->datum #'?name))))
       		     (NAME->VALUE	(datum->syntax #'?name
						       (%name->tovalue-name   (syntax->datum #'?name))))
       		     (VALUE->NAME	(datum->syntax #'?name
						       (%name->fromvalue-name (syntax->datum #'?name))))
       		     (STX-NAME		(datum->syntax #'?name
						       (%name->stx-name       (syntax->datum #'?name)))))
	 #'(begin
...

(define-syntax define-c-ior-flags
  (lambda (stx)
    (define (%name->enum-name sym)
      (string->symbol (string-append "enum-" (symbol->string sym))))
    (define (%name->tovalue-name sym)
